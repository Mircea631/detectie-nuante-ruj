...
st.set_page_config(page_title="💄 Detectare Nuanță Ruj Avon", layout="centered")
...
st.title("💋 Recunoaștere Nuanță Ruj Avon")
uploaded_files = st.file_uploader("📸 Încarcă imagini", type=["jpg", "png"], accept_multiple_files=True)

if uploaded_files and ROBOFLOW_API_KEY:
    for i, img_file in enumerate(uploaded_files):
        with st.expander(f"📷 Imagine #{i+1}: {img_file.name}", expanded=True):
            ...
            mask = segment_lips_with_roboflow(img, ROBOFLOW_API_KEY, ROBOFLOW_MODEL_URL)
